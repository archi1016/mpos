...
$DB_FOLDER = MOPO_SERVER_FOLDER.FOLDER_DB_SALES;
...
$DB_FILE_INDEX_FLAVOR = $DB_FOLDER.'/index_'.QUERY_TYPE_FLAVOR.FILE_TSV_EXT_NAME;
$DB_FILE_INDEX_FLAVOR_GROUP = $DB_FOLDER.'/index_'.QUERY_TYPE_FLAVOR_GROUP.FILE_TSV_EXT_NAME;
...
switch ($qtv) {
	...
	case QUERY_TYPE_FLAVOR:
		$CURRENT_FLAVOR_INDEX = RETURN_FIND_ROW_INDEX($DB_FILE_INDEX_FLAVOR, $qiv, $FLAVOR_INFORMATION, TSV_COMMON_ID, $FLAVOR_INFORMATION_ROWS);
		if (-1 != $CURRENT_FLAVOR_INDEX) {
			$current_row = $FLAVOR_INFORMATION[$CURRENT_FLAVOR_INDEX];
			$page_title = RETURN_ITEM_GROUP_NAME($CURRENT_FLAVOR_INDEX, $FLAVOR_INFORMATION).' / '.$current_row[TSV_COMMON_ID].': '.$current_row[TSV_COMMON_NAME].' ('.$yrv.'-'.RETURN_CONV_INT_TO_NUM_2X($mnv).')';
			$a[0] = WEB_API_DUMP_REPORT_ITEM;
			$a[1] = $ACCESS_KEY;
			$a[2] = $SECTION_ID;
			$a[3] = $yrv;
	...
	case QUERY_TYPE_FLAVOR_GROUP:
		$CURRENT_FLAVOR_GROUP_INDEX = RETURN_FIND_ROW_INDEX($DB_FILE_INDEX_FLAVOR_GROUP, $qiv, $FLAVOR_GROUP_INFORMATION, TSV_COMMON_ID, $FLAVOR_GROUP_INFORMATION_ROWS);
		if (-1 != $CURRENT_FLAVOR_GROUP_INDEX) {
			$current_row = $FLAVOR_GROUP_INFORMATION[$CURRENT_FLAVOR_GROUP_INDEX];
			$page_title = RETURN_ITEM_GROUP_NAME($CURRENT_FLAVOR_GROUP_INDEX, $FLAVOR_GROUP_INFORMATION).' / '.$current_row[TSV_COMMON_NAME].' ('.$yrv.'-'.RETURN_CONV_INT_TO_NUM_2X($mnv).')';
			$a[0] = WEB_API_DUMP_REPORT_GROUP;
			$a[1] = $ACCESS_KEY;
			$a[2] = $SECTION_ID;
			$a[3] = $yrv;
...
if ('' != $page_title) {
	...
	switch ($qtv) {
		...
		case QUERY_TYPE_FLAVOR:
			$gpi[$gpi_count][GROUP_PANEL_NAME] = '口味依品項';
			$h = RETURN_QUERY_FORM_HEADER('查詢');
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(QUERY_TYPE_NAME, $qtv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(YEAR_NAME, $yrv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(MONTH_NAME, $mnv);
			$h .= RETURN_ITEM_OPTIONS($DB_FILE_INDEX_FLAVOR, $qiv);
			$h .= RETURN_QUERY_FORM_FOOTER();
			$gpi[$gpi_count][GROUP_PANEL_BUTTONS] = $h;
			++$gpi_count;
			break;
		...
		case QUERY_TYPE_FLAVOR_GROUP:
			$gpi[$gpi_count][GROUP_PANEL_NAME] = '口味依欄位';
			$h = RETURN_QUERY_FORM_HEADER('查詢');
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(QUERY_TYPE_NAME, $qtv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(YEAR_NAME, $yrv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(MONTH_NAME, $mnv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(KEYWORD_NAME, $kwv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(FILTER_TYPE_NAME, $ftv);
			$h .= RETURN_ITEM_OPTIONS($DB_FILE_INDEX_FLAVOR_GROUP, $qiv);
			$h .= RETURN_QUERY_FORM_FOOTER();
			$gpi[$gpi_count][GROUP_PANEL_BUTTONS] = $h;
			++$gpi_count;
...
if (SIDE_NAV_INIT('')) {
	if ('' != $page_title) {	
		...
		PAGE_ADD('<h2>口味依品項</h2>');
			REPORT_QUERY_INIT(RETURN_HIDDEN_INPUT_ELEMENT(QUERY_TYPE_NAME, QUERY_TYPE_FLAVOR));
			$qpi_count = 0;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '年';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_YEAR_OPTIONS($yrv);
			++$qpi_count;
			...
			++$qpi_count;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '項目';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_ITEM_OPTIONS($DB_FILE_INDEX_FLAVOR);
			++$qpi_count;		
			REPORT_QUERY_CLOSE($qpi);
		
		PAGE_ADD('<h2>口味依欄位</h2>');
			REPORT_QUERY_INIT(RETURN_HIDDEN_INPUT_ELEMENT(QUERY_TYPE_NAME, QUERY_TYPE_FLAVOR_GROUP));
			$qpi_count = 0;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '年';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_YEAR_OPTIONS($yrv);
			++$qpi_count;
			...
			++$qpi_count;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '項目';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_ITEM_OPTIONS($DB_FILE_INDEX_FLAVOR_GROUP);
			++$qpi_count;
			REPORT_QUERY_CLOSE($qpi);
	}
	SIDE_NAV_CLOSE();
