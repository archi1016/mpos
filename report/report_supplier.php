...
$DB_FOLDER = MOPO_SERVER_FOLDER.FOLDER_DB_SUPPLIER;
$DB_FILE_INDEX_GOODS = $DB_FOLDER.'/index_'.QUERY_TYPE_GOODS.FILE_TSV_EXT_NAME;
$DB_FILE_INDEX_GOODS_SUPPLIER = $DB_FOLDER.'/index_'.QUERY_TYPE_GOODS_SUPPLIER.FILE_TSV_EXT_NAME;
$DB_FILE_INDEX_INGREDIENT = $DB_FOLDER.'/index_'.QUERY_TYPE_INGREDIENT.FILE_TSV_EXT_NAME;
$DB_FILE_INDEX_INGREDIENT_SUPPLIER = $DB_FOLDER.'/index_'.QUERY_TYPE_INGREDIENT_SUPPLIER.FILE_TSV_EXT_NAME;
...
switch ($qtv) {
	...
	case QUERY_TYPE_GOODS_SUPPLIER:
		$CURRENT_GOODS_SUPPLIER_INDEX = RETURN_FIND_ROW_INDEX($DB_FILE_INDEX_GOODS_SUPPLIER, $qiv, $GOODS_SUPPLIER_INFORMATION, TSV_COMMON_ID, $GOODS_SUPPLIER_INFORMATION_ROWS);
		if (-1 != $CURRENT_GOODS_SUPPLIER_INDEX) {
			$current_row = $GOODS_SUPPLIER_INFORMATION[$CURRENT_GOODS_SUPPLIER_INDEX];
			$page_title = RETURN_ITEM_GROUP_NAME($CURRENT_GOODS_SUPPLIER_INDEX, $GOODS_SUPPLIER_INFORMATION).' / '.$current_row[TSV_COMMON_NAME].' ('.$yrv.'-'.RETURN_CONV_INT_TO_NUM_2X($mnv).')';
	...
	case QUERY_TYPE_INGREDIENT_SUPPLIER:
		$CURRENT_INGREDIENT_SUPPLIER_INDEX = RETURN_FIND_ROW_INDEX($DB_FILE_INDEX_INGREDIENT_SUPPLIER, $qiv, $INGREDIENT_SUPPLIER_INFORMATION, TSV_COMMON_ID, $INGREDIENT_SUPPLIER_INFORMATION_ROWS);
		if (-1 != $CURRENT_INGREDIENT_SUPPLIER_INDEX) {
			$current_row = $INGREDIENT_SUPPLIER_INFORMATION[$CURRENT_INGREDIENT_SUPPLIER_INDEX];
			$page_title = RETURN_ITEM_GROUP_NAME($CURRENT_INGREDIENT_SUPPLIER_INDEX, $INGREDIENT_SUPPLIER_INFORMATION).' / '.$current_row[TSV_COMMON_NAME].' ('.$yrv.'-'.RETURN_CONV_INT_TO_NUM_2X($mnv).')';
			$a[0] = WEB_API_DUMP_REPORT_GROUP;
			$a[1] = $ACCESS_KEY;
			$a[2] = $SECTION_ID;
			$a[3] = $yrv;
...
if ('' != $page_title) {
	...
	switch ($qtv) {
		...
		case QUERY_TYPE_GOODS_SUPPLIER:
			...
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(KEYWORD_NAME, $kwv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(FILTER_TYPE_NAME, $ftv);
			$h .= RETURN_ITEM_OPTIONS($DB_FILE_INDEX_GOODS_SUPPLIER, $qiv);
			$h .= RETURN_QUERY_FORM_FOOTER();
			$gpi[$gpi_count][GROUP_PANEL_BUTTONS] = $h;
			++$gpi_count;
		...
		case QUERY_TYPE_INGREDIENT_SUPPLIER:
			...
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(KEYWORD_NAME, $kwv);
			$h .= RETURN_HIDDEN_INPUT_ELEMENT(FILTER_TYPE_NAME, $ftv);
			$h .= RETURN_ITEM_OPTIONS($DB_FILE_INDEX_INGREDIENT_SUPPLIER, $qiv);
			$h .= RETURN_QUERY_FORM_FOOTER();
			$gpi[$gpi_count][GROUP_PANEL_BUTTONS] = $h;
			++$gpi_count;
...
if (SIDE_NAV_INIT('')) {
	if ('' != $page_title) {	
		...
		PAGE_ADD('<h2>商品依欄位</h2>');
			REPORT_QUERY_INIT(RETURN_HIDDEN_INPUT_ELEMENT(QUERY_TYPE_NAME, QUERY_TYPE_GOODS_SUPPLIER));
			$qpi_count = 0;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '年';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_YEAR_OPTIONS($yrv);
			++$qpi_count;
			...
			++$qpi_count;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '項目';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_ITEM_OPTIONS($DB_FILE_INDEX_GOODS_SUPPLIER);
			++$qpi_count;
			REPORT_QUERY_CLOSE($qpi);
		...
		PAGE_ADD('<h2>原物料依欄位</h2>');
			REPORT_QUERY_INIT(RETURN_HIDDEN_INPUT_ELEMENT(QUERY_TYPE_NAME, QUERY_TYPE_INGREDIENT_SUPPLIER));
			$qpi_count = 0;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '年';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_YEAR_OPTIONS($yrv);
			++$qpi_count;
			...
			++$qpi_count;
			$qpi[$qpi_count][QUERY_PANEL_NAME] = '項目';
			$qpi[$qpi_count][QUERY_PANEL_BUTTONS] = RETURN_REPORT_ITEM_OPTIONS($DB_FILE_INDEX_INGREDIENT_SUPPLIER);
			++$qpi_count;
			REPORT_QUERY_CLOSE($qpi);
	}
	SIDE_NAV_CLOSE();
